...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        // Call backend to initialize system or get welcome text (optional)
        ...
        // If backend returns text, use it — else use default greeting
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const errorText =
        "Oops! Something went wrong while connecting to the assistant. Please try again later.";
      setMessages((prev) =>
        prev.map((msg) =>
